...
module.exports = (options) ->
  ...
  fetch = (callback) ->
    request.get("#{options.apiUrl}/api/v1/xapp_token").query(
  ...
  (req, res, next) ->
    unless module.exports.token?
      fetch (err, token) ->
        res.locals.takomanXappToken = module.exports.token = token
        next()
    else
      res.locals.takomanXappToken = module.exports.token
      next()
