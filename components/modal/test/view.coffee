  ...
  describe '#open', ->
    describe '#setup', ->
      it 'renders the inner template function', ->
        @view.$el.html().should.match /.*Requires a template.*/
      ...
      it 'sets the default classes', ->
        ...
        classes.split(' ').should.containEql 'is-fade-in'
        classes.split(' ').should.containEql 'has-backdrop'
      ...
      it 'should be able to accept backdrop and transition options', ->
        ...
        classes.split(' ').should.containEql 'is-fade-in'
        classes.split(' ').should.containEql 'has-nobackdrop'

        modal = new ModalView($container: $('#modal-container'), backdrop: false, transition: 'slide')
        classes = modal.$el.attr('class')
        classes.split(' ').should.containEql 'is-slide-in'
        classes.split(' ').should.containEql 'has-nobackdrop'
